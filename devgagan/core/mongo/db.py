...
async def get_all_sessions():
    """Return every document in MongoDB that has a non-null session string stored."""
    sessions = []
    async for doc in db.find({"session": {"$exists": True, "$ne": None}}):
        sessions.append(doc)
    return sessions

# Replacement words functions - save to MongoDB permanently
async def set_replacement_words(user_id, replacement_words):
    """Save replacement words dictionary to MongoDB"""
    data = await get_data(user_id)
    if data and data.get("_id"):
        await db.update_one({"_id": user_id}, {"$set": {"replacement_words": replacement_words}})
    else:
        await db.insert_one({"_id": user_id, "replacement_words": replacement_words})

async def get_replacement_words(user_id):
    """Get replacement words dictionary from MongoDB"""
    data = await get_data(user_id)
    if data and data.get("replacement_words"):
        return data.get("replacement_words", {})
    return {}

# Delete words functions - save to MongoDB permanently
async def set_delete_words(user_id, delete_words):
    """Save delete words list to MongoDB"""
    data = await get_data(user_id)
    if data and data.get("_id"):
        await db.update_one({"_id": user_id}, {"$set": {"delete_words": list(delete_words)}})
    else:
        await db.insert_one({"_id": user_id, "delete_words": list(delete_words)})

async def get_delete_words(user_id):
    """Get delete words list from MongoDB"""
    data = await get_data(user_id)
    if data and data.get("delete_words"):
        return set(data.get("delete_words", []))
    return set()
