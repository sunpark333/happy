   ...
   if update_channel:
      try:
         ...
         if user.status == "kicked":
            await message.reply_text("You are Banned. Contact -- @PUSHPA_BHAV")
            return 1
      except UserNotParticipant:
        caption = f"Join our channel to use the bot"
        await message.reply_photo(photo="https://postimg.cc/HcP4R8v8",caption=caption, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Join Now...", url=f"{url}")]]))
        return 1
      except Exception:
         await message.reply_text("Something Went Wrong. Contact us @PUSHPA_BHAV...")
         return 1
...
def video_metadata(file):
    default_values = {'width': 1280, 'height': 720, 'duration': 1}
    try:
        # Check if file exists first
        if not os.path.exists(file):
            print(f"File not found for metadata: {file}")
        ...
        if width <= 0 or height <= 0:
            width = 1280
            height = 720
            print(f"Invalid dimensions detected, using defaults")

        if fps <= 0:
            print(f"Invalid FPS detected: {fps}")
            vcap.release()
            return default_values  
        ...
        if duration <= 0:
            duration = 1
            print(f"Invalid duration calculated, using default")
